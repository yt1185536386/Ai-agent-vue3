...
> 范围:`packages/NestJS`(BFF 网关,:26011)、`packages/model-gateway`(Java 模型网关,:26015)
...
|---|---|---|
| CORS | `CORS_ORIGINS` 白名单(缺省仅本地 26012/26013) | `gateway.cors.allowed-origins` 白名单 |
...
**NestJS(.env)**：`PORT`(26011)、`JWT_SECRET`、`JWT_EXPIRES_IN`(默认 7d)、`NESTJS_SERVICE_KEY`(与 Java `GATEWAY_SERVICE_KEY` 同值)、`CORS_ORIGINS`、`DB_SYNCHRONIZE`

**Java(application.yml)**：`PORT`(26015)、`GATEWAY_SERVICE_KEY`、`CORS_ORIGINS`、`gateway.jwt.secret`、`gateway.invoke.connect/read-timeout-ms`、`gateway.seed.enabled`
