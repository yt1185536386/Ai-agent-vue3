...
def _extract_readable_error(e: Exception) -> str:
    """把异常转成对前端友好的可读信息。

    优先从 openai.BadRequestError 之类的原始 400 响应体里剥出 message
    (后端 model-gateway 会把上游错误包成 {error:{message}}),
    拿不到时回退到异常自身的定位描述。
    """
    text = str(e)
    # 递归剥离形如 {...} 的 JSON 外壳,取最内层真实原因
    for _ in range(3):
        start = text.find("{")
        end = text.rfind("}")
        ...
        try:
            ...
            break
        # 兼容 {error:{message}} / {message:}
        nxt = None
        if isinstance(obj, dict):
            err = obj.get("error")
            nxt = err.get("message") if isinstance(err, dict) else obj.get("message")
        if not isinstance(nxt, str) or not nxt.strip() or nxt == text:
            break
        text = nxt
        # 继续剥下一层(openai 里 message 还可能是 gateway 的字符串转义 JSON)
        if "{" in text and "}" in text:
            continue
        break
    if not text.strip():
        return "服务内部错误,请稍后重试"
    return f"抱歉,暂时无法完成请求:{text.strip()[:300]}"
...
async def run_agent_stream(agent, graph_input, model: str,
    ...
    异常契约:图中任一步出错(模型 400/工具崩溃/外部知识库不可用)都不允许
    让 SSE 连接直接断开——那样前端 only 收到 ERR_ABORTED,无任何可读信息。
    捕获后改为 yield 一个 error 事件(前端 sse.ts 已处理 event=="error"),
    再正常结束,保证前端拿到明确原因而非"无响应"。
    """
    logger.info("run_agent_stream start model=%s", model)
    ...
    try:
        ...
        # 任何环节异常(模型 400/工具崩溃/知识库不可用)都不让 SSE 连接断掉,
        # 而是 yield 一个 error 事件,前端据此显示可读错误(sse.ts/streamChat)。
        # 注意:此时不能保证有 graph state 可查,降到只发 error。
        logger.warning("run_agent_stream failed: %s %s", type(e).__name__, e)
        msg = _extract_readable_error(e)
        if collector is not None and not collector["text"]:
            collector["text"] = msg
        yield sse_event("error", {"message": msg})
    else:
        # 只有图正常走完才查终态(拒绝文案 / HITL 待审批);
        # 异常路径不查,避免在坏状态下再触发一次异常。
        if config:
