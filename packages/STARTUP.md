...
| 项目 | 目录 | 端口 | 启动命令 | 端口配置 |
|---|---|---|---|---|
| AI 服务(Agent 编排) | `ai-service/` | 6010 | uvicorn(见下) | `ai-service/.env` 的 `PORT` |
| NestJS 网关(鉴权 + 业务) | `NestJS/` | 6011 | `npm run start:dev` | `NestJS/.env` 的 `PORT` |
| Vue 前端 | `my-vue-app-ts/` | 6012 | `npm run dev` | `my-vue-app-ts/.env` 的 `VITE_PORT` |
| 管理端 UI | `ServerManegeUI/` | 6013 | `npm run dev` | `ServerManegeUI/vite.config.ts` |
| Java 模型网关(唯一模型出口) | `model-gateway/` | 6015 | `powershell -File run-mysql.ps1` | `model-gateway` application.yml 的 `PORT` |
| eval 评测包 | `eval/` | —(不占端口) | `run_eval.py`(见下) | 复用 `ai-service/.env` |

> 6014 空出来不用:my-vue-app-ts 的 vite 在 6012 被占时会自增到 6013/6014。

## 调用链

```
浏览器 (http://localhost:6012)
  → Vite 代理 /api
    → NestJS 网关 (6011,JWT 鉴权 + 业务接口,唯一用户权限权威)
      → ai-service (6010,harness:ReAct 循环 + SSE)
        → Java model-gateway (6015,唯一模型出口:限流/熔断/计量/渠道调度)
          → 上游模型提供方 (company / 百炼,真实 key 只配置在 channels 表)
```

管理端 (http://localhost:6013):登录走 NestJS(共享 JWT),
...
- 模型上游可达:公司 `http://172.18.200.114:3000/v1` 或百炼 dashscope
  ...
  由各服务 `.env` 指向 6015;`run-mysql.ps1` 首次启动会播种渠道)
...
```bash
cd ai-service
.venv/Scripts/python -m uvicorn app.main:app --reload --host 127.0.0.1 --port 6010
```
...
健康检查:`curl http://localhost:6010/health`
...
读取 `NestJS/.env` 的 `PORT=6011`;启动日志出现
...
Vite 读取 `my-vue-app-ts/.env` 的 `VITE_PORT=6012`,
代理目标取 `NESTJS_PORT=6011`。
...
- `NestJS/src/main.ts`:`process.env.PORT ?? 6011`
- `NestJS/src/chat/chat.service.ts`:`AI_SERVICE_URL ?? 'http://localhost:6010'`
- `my-vue-app-ts/vite.config.ts`:`VITE_PORT ?? '6012'`、`NESTJS_PORT ?? '6011'`
...
```bash
curl http://localhost:6010/health   # AI 服务
curl http://localhost:6011          # NestJS(返回根响应)
# 前端:浏览器打开 http://localhost:6012
```

端到端:在 Vue 应用里发一条对话,链路应为 `6012 → 6011 → 6010`;
...
`dev-err.log`;重启前先 `netstat -ano | findstr :6010` 检查端口是否被旧进程占用。
